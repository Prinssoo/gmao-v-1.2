...
class InterventionRequestController extends Controller
{
    ...
    {
        if (!$request->user()->can('intervention_request:view_any')) {
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        // Déterminer le site_id selon l'asset
        $siteId = $request->user()->current_site_id;
        ...
        // Nettoyer les IDs non pertinents
    ...
    {
        if (!$request->user()->can('intervention_request:view')) {
            abort(403, 'Accès non autorisé');
        }

        // Vérifier l'accès au site (admin peut voir tous les sites)
    ...
    public function update(Request $request, InterventionRequest $interventionRequest): JsonResponse
    {
        ...
        // On ne peut modifier que si en attente
    ...
    public function destroy(Request $request, InterventionRequest $interventionRequest): JsonResponse
    {
        ...
        // On ne peut supprimer que si en attente
        if (!$interventionRequest->isPending()) {
            return response()->json([
    ...
    /**
     * Valider ou rejeter une demande (legacy - garde pour compatibilité)
     */
    ...
    /**
     * Convertir une DI approuvée en OT
     */
    public function convertToWorkOrder(Request $request, InterventionRequest $interventionRequest): JsonResponse
    {
        ...
        // Mapper l'urgence vers la priorité si non fournie
        $priorityMap = [
            'low' => 'low',
            'medium' => 'medium',
            'high' => 'high',
        ...
        $workOrder = DB::transaction(function () use ($interventionRequest, $validated, $request, $priorityMap) {
            // Créer l'OT avec support asset_type
    ...
    public function stats(Request $request): JsonResponse
    {
        ...
        // Stats par urgence (pending uniquement)
        $byUrgency = [
        ...
        // Top équipements avec le plus de DI
        ...
        // Top camions avec le plus de DI
    ...
    /**
     * Liste des DI pour un équipement spécifique
     */
    public function forEquipment(Request $request, Equipment $equipment): JsonResponse
    {
        ...
        $requests = InterventionRequest::where('equipment_id', $equipment->id)
    ...
    /**
     * Liste des DI pour un camion spécifique
     */
    public function forTruck(Request $request, Truck $truck): JsonResponse
    {
        ...
        $requests = InterventionRequest::where('truck_id', $truck->id)
            ->with(['requestedBy', 'validatedBy', 'workOrder'])
            ->orderByDesc('created_at')
            ->paginate($request->per_page ?? 10);
