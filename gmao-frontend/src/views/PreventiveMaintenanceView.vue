...
// Unités de compteur prédéfinies pour les camions
...
function onAssetTypeChange() {
  ...
  form.value.truck_id = ''
  // Pré-remplir l'unité de compteur pour les camions
...
async function generateWorkOrder(plan) {
  if (!confirm(`Générer un OT pour "${plan.name}" ?`)) return
  saving.value = true
  try {
    const response = await api.post(`/preventive-maintenances/${plan.id}/generate-wo`)
      ...
      <div class="modal modal-lg">
        ...
        <form @submit.prevent="createPlan" class="modal-body">
          ...
          <div class="form-section">
            ...
            <!-- Info pour maintenance basée km -->
